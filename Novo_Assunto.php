...
if ($_SERVER["REQUEST_METHOD"] == "POST") 
{
  if (isset($_POST["assunto"])) 
  {
    ...
    $sql = "INSERT INTO $ass (nomeAss, idUser) VALUES ('$assunto', '$user_id')";
    ...
    if($c > 0)
    {
      for($x = 0; $x < $c; $x++) 
      {
        $exerc = $_POST['exerc'];
        $exerc = $exerc[$x];
        $sql = "INSERT INTO $listex (idExe, idAss) VALUES ('$exerc', '$last_id')";
        if(!mysqli_query($conn,$sql))
        {
    ...
    <div class="row">
      ...
      <?php if($info): ?>
        ...
        <form action="<?php echo $_SERVER["PHP_SELF"] ?>" method="POST">
          ...
          <br>
          <button type="submit" class="btn btn-success active">Criar</button>
        </form>
